...
public class PaymentController {
    ...
    vnpayReturn(@RequestParam Map<String, String> params) {
        ...
        try {
            if ("00".equals(responseCode)) {
                ...
                // Nếu booking có voucher → đánh dấu used
    ...
    public ResponseEntity<?> createVnPayPayment(@Valid @RequestBody PaymentDTO paymentDTO)
            throws InvalidException {
        Booking booking = this.bookingSer.getById(paymentDTO.getId_booking());
        if (booking.getPayment() != null) {
            throw new InvalidException("Booking đã thanh toán");
        ...
        try {
            // Tạo link VNPay có tính voucher
            Map<String, Object> result =
                    paymentSer.createVNPayWithVoucher(
                            paymentDTO.getId_booking(),
                            paymentDTO.getUserVoucherId()
                    );
            ...
            return ResponseEntity.internalServerError()
                    .body("Lỗi khi tạo link thanh toán VNPay: " + e.getMessage());
        }
    }
